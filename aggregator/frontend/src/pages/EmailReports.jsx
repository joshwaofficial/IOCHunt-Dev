...
export default function EmailReports() {
  ...
  const [formData, setFormData] = useState({
    name: '', recipients: '', cron_expr: '0 8 * * 1', duration: 24, machine: '', severity: '', category: '', include_fw: true, enabled: true
  });
  ...
  const openNewForm = () => {
    setEditId(null);
    setFormData({ name: '', recipients: '', cron_expr: '0 8 * * 1', duration: 24, machine: '', severity: '', category: '', include_fw: true, enabled: true });
    setShowForm(true);
  ...
  const editSchedule = (s) => {
    setEditId(s.id);
    setFormData({
      name: s.name, recipients: s.recipients, cron_expr: s.cron_expr, duration: s.duration, machine: s.machine || '', severity: s.severity || '', category: s.category || '', include_fw: s.include_fw === 1, enabled: s.enabled === 1
    });
    setShowForm(true);
    setSchedMsg({ text: '', type: '' });
  };
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--mono)' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(90deg, rgba(37,99,235,0.06) 0%, rgba(37,99,235,0) 100%)' }}>
                    ...
                    {!isAggregator && <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'right' }}>Actions</th>}
                  </tr>
                </thead>
                <tbody>
                  {schedules.map(s => (
