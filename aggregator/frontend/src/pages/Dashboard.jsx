...
export default function Dashboard() {
  const { range, machine, setMachine } = useFilter();
  ...
  useEffect(() => {
    fetchInitialEvents(range, machine);
    connectSSE(machine, range);
    return () => disconnectSSE();
  }, [range, machine, connectSSE, disconnectSSE, fetchInitialEvents]);

  // React Query for aggregate backend stats
  const { data: stats, isLoading: statsLoading, error: statsError } = useQuery({
    queryKey: ['stats', range, machine],
    queryFn: async () => {
  ...
  const { data: topoData } = useQuery({
    queryKey: ['topology', range, machine],
    queryFn: async () => {
      const res = await axios.get(`/api/events/network/topology?hours=${range}&machine=${machine}`);
      return res.data;
    }
  });
  ...
  if (stats && stats.critical && stats.critical.length > 0) {
    ...
    if (stats.byMachineSev && stats.byMachineSev.length > 0) {
      ...
      if (critRows.length > 0) {
        const topRow = critRows.reduce((prev, curr) => prev.n > curr.n ? prev : curr);
        topMachineName = topRow.machine;
        topCount = topRow.n;
      }
    }
    
    let event = stats.critical.find(e => e.machine === topMachineName || e.target_machine === topMachineName);
    if (!event) {
      event = stats.critical[0];
      topMachineName = event.machine || event.target_machine || 'Unknown';
      topCount = stats.totalCritical || stats.critical.length;
    }
