...
export default function AllLogs() {
  ...
  const [range, setRange] = useState(localStorage.getItem('iochunt-logs-range') || '24');
  ...
  useEffect(() => {
    const fetchMachines = async () => {
      try {
        const res = await axios.get('/api/events/machines');
        if (Array.isArray(res.data)) {
          setAvailableMachines(res.data.map(m => m.id));
        }
      } catch (err) {
  ...
  }, [range, machine, severityFilter, categoryFilter, searchTerm, showNoise, page, perPage, customFrom, customTo]);
