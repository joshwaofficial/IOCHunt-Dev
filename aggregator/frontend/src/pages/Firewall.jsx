...
export default function Firewall() {
  ...
  }, [from, to, action, service, ip, severity, device, page, perPage, liveMode, flowFilter]);
  ...
  const fetchDevices = async () => {
    try {
      const res = await axios.get('/api/firewall/devices');
      setDevices(res.data || []);
    } catch (e) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const params = { 
        ...
        severity, device, limit: perPage, offset 
      };
  ...
  const fetchLive = async () => {
    try {
      const res = await axios.get('/api/firewall/live', {
        params: { last_id: lastLiveId, limit: 50 }
      });
    ...
    <div style={{ width: '100%', paddingBottom: '40px', position: 'relative' }}>
      ...
      {!liveMode && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '16px', marginBottom: '24px' }}>
          <FirewallAlerts from={from} to={to} device={device} />
          <FirewallTopology 
            from={from} to={to} action={action} service={service} ip={ip} device={device} 
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--mono)' }}>
                <thead style={{ position: 'sticky', top: 0, zIndex: 1, background: 'var(--surface)' }}>
                  <tr style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(90deg, rgba(37,99,235,0.06) 0%, rgba(37,99,235,0) 100%)' }}>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Dst Port</th>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Proto</th>
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Sent</th>
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Recvd</th>
                  </tr>
                </thead>
                <tbody>
                  {liveEvents.map((a, i) => {
                    ...
                    return (
                      <tr key={a.id} className="hover-row" onClick={() => setSelectedEvent(a)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                        ...
                        <td style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text)' }}>{esc(a.dst_port)}</td>
                        ...
                        <td style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text)' }}>{esc(a.proto)}</td>
                        <td style={{ padding: '14px 16px', fontSize: '10px', color: 'var(--muted)' }}>{esc(a.sentbyte)}</td>
                        <td style={{ padding: '14px 16px', fontSize: '10px', color: 'var(--muted)' }}>{esc(a.rcvdbyte)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                    ...
                    {data.events.map((a, i) => {
                      ...
                      return (
                        <tr key={i} onClick={() => setSelectedEvent(a)} style={{ cursor: 'pointer' }}>
                          <td className="et" style={{ fontFamily: 'var(--mono)', color: 'var(--muted2)', whiteSpace: 'nowrap' }}>{a.ts ? new Date(a.ts).toLocaleString('sv-SE').slice(0, 16).replace('T', ' ') : ''}</td>
                          <td style={{ fontFamily: 'var(--mono)', color: isPrivateIp(a.src_ip) ? '#60a5fa' : '#f97316', fontWeight: 600 }}>{esc(a.src_ip)}</td>
