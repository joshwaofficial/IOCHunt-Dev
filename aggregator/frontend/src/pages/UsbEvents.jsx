...
export default function UsbEvents() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await axios.get('/api/events/usb', { params });
        setData(res.data);
        
        if (!machine) {
          const uniqueMachines = [...new Set(res.data.map(a => a.target_machine || a.machine))].filter(Boolean);
    ...
    fetchData();
  }, [range, machine, customFrom, customTo]);

  const filteredData = data.filter(a => {
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      const matchSearch = Object.values(a).some(v => v !== null && v !== undefined && String(v).toLowerCase().includes(term));
  ...
  const highRisk = filteredData.filter(a => ['high', 'critical'].includes((a.severity || '').toLowerCase())).length;
  const uniqueDrives = new Set(filteredData.map(a => a.drive)).size;
  const uniqueDevices = new Set(filteredData.map(a => a.device)).size;
  ...
  return (
    <div style={{ width: '100%', paddingBottom: '40px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '26px', fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text)', margin: 0 }}>USB & Removable Media</h2>
          <p style={{ fontSize: '11px', color: 'var(--muted)', margin: '6px 0 0', fontFamily: 'var(--mono)' }}>Monitor USB insertions, drive mappings, and removable media activity.</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <PremiumCard value={total} label="Total Events" color="#3b82f6" icon="usb" subtitle="Active tracking" />
        <PremiumCard value={highRisk} label="High Risk" color="#ef4444" icon="warning" subtitle="Elevated risk" />
        <PremiumCard value={uniqueDrives} label="Drives" color="#22c55e" icon="save" subtitle="Unique drive letters" />
        <PremiumCard value={uniqueDevices} label="Devices" color="#f97316" icon="devices" subtitle="Unique hardware" />
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="material-symbols-outlined" style={{ fontSize: '18px', color: 'var(--muted)' }}>usb</span>
          <h2 style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--text)', fontFamily: 'var(--mono)', margin: 0 }}>USB Activity</h2>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--muted)' }}>No USB events in this window</div>
        ) : (
          <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--mono)' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(90deg, rgba(37,99,235,0.06) 0%, rgba(37,99,235,0) 100%)' }}>
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Sev</th>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Drive</th>
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Label</th>
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Size</th>
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Findings</th>
                ...
                <tbody>
                  {paginatedData.map((a, i) => {
                    ...
                    let acCol = 'var(--text)';
                    if (a.action === 'Threat Found') acCol = '#ef4444';
                    else if ((a.action || '').includes('Autorun')) acCol = '#f97316';
                    else if (a.action === 'Removed') acCol = 'var(--muted)';
                    else if (a.action === 'Inserted') acCol = '#22c55e';
                    ...
                    return (
                      <tr key={i} className="hover-row" onClick={() => setSelectedEvent(a)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                        ...
                        <td style={{ padding: '14px 16px', fontSize: '12px', fontWeight: 600, color: acCol, fontFamily: 'var(--sans)' }}>
                          {esc(a.action || 'Inserted')}
                        </td>
                        <td style={{ padding: '14px 16px' }}>
                          <span className="mn" style={{ fontSize: '11px', color: '#3b82f6' }}>{esc(a.machine || '?')}</span>
                        </td>
                        <td style={{ padding: '14px 16px', fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--accent)' }}>{esc(a.drive || '-')}</td>
                        <td style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text)' }}>{esc(a.label || '')}</td>
                        <td style={{ padding: '14px 16px', fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--muted)' }}>{esc(a.size || '')}</td>
                        <td style={{ padding: '14px 16px' }}>
                          {a.findings > 0 ? (
                            <span style={{ display: 'inline-block', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 700, background: 'rgba(239,68,68,0.1)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.25)' }}>{a.findings}</span>
                          ) : (
                            <span style={{ color: 'var(--muted)', fontSize: '11px' }}>0</span>
                          )}
                        </td>
                        <td style={{ padding: '14px 16px', fontSize: '10px', color: 'var(--muted2)' }}>
                          {a.ts ? new Date(a.ts).toLocaleString('sv-SE').slice(0, 16) : ''}
                        </td>
                      </tr>
                    );
                  })}
