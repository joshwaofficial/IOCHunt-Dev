...
export default function AdAttacks() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const params = { machine, page, limit: perPage, search: searchTerm, severity: severityFilter };
        if (range === 'custom') {
        ...
        if (!machine) {
          const uniqueMachines = [...new Set((res.data.events || []).map(a => a.target_machine || a.machine))].filter(Boolean);
    ...
    fetchData();
  }, [range, machine, page, perPage, searchTerm, severityFilter, customFrom, customTo, actorFilter, typeFilter, sortFilter, protocolFilter, tacticFilter]);
                  ...
                  {paginatedData.map((a, i) => {
                    ...
                    return (
                      <tr key={i} className="hover-row" onClick={() => setSelectedEvent(a)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                        ...
                        <td style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--text)', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={esc(a.description)}>
                          {esc(a.description || '')}
                        </td>
                        <td style={{ padding: '14px 16px' }}>
