...
export default function Policy() {
  ...
  const renderPolicyEditor = () => {
    ...
    return (
      <div style={{ pointerEvents: isAggregator ? 'none' : 'auto', opacity: isAggregator ? 0.8 : 1 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '4px' }}>
          {!isAggregator && !editingGroupId && selectedMachine && machinePolicyData?.policy_source === 'machine' && (
            <button
  ...
  return (
    <div style={{ width: '100%', paddingBottom: '40px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '26px', fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text)', margin: 0 }}>Policy Management {isAggregator && <span style={{ fontSize: '11px', background: '#ef4444', color: '#fff', padding: '2px 8px', borderRadius: '4px', marginLeft: '10px' }}>READ ONLY</span>}</h2>
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '16px' }}>
          ...
          {!isAggregator && <button onClick={createGroup} style={{ background: 'var(--accent)', color: '#fff', border: 'none', padding: '6px 14px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 12px rgba(37,99,235,0.2)', transition: 'all 0.2s' }}>+ New Group</button>}
        </div>

        {groups.length === 0 ? (
          <div style={{ fontSize: '12px', color: 'var(--muted)', padding: '8px 0 14px' }}>No groups yet. {!isAggregator && "Create one to push policy to multiple machines at once."}</div>
        ) : (
          groups.filter(g => {
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center' }}>
                  {g.machines.map(m => (
                    <span key={m} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(59,130,246,.08)', border: '1px solid rgba(59,130,246,.25)', color: '#60a5fa', fontFamily: 'var(--mono)', fontSize: '11px', padding: '3px 10px', borderRadius: '6px', fontWeight: 600 }}>
                      {m}
                      {!isAggregator && <button onClick={() => assignMachineToGroup(m, '')} style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', padding: 0, fontSize: '12px', lineHeight: 1 }}>✕</button>}
                    </span>
                  ))}
                  {!isAggregator && (
