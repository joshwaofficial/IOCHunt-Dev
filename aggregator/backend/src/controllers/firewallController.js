...
exports.getFirewallStats = async (req, res) => {
  try {
    const { 
      ...
      action, service, ip, src_ip, dst_ip, severity, device, limit = 200, offset = 0 
    } = req.query;
    ...
    if (action) { conds.push(`action=$${pIdx++}`); p.push(action); }
    if (service) {
...
exports.getTopology = async (req, res) => {
  try {
    const { 
      from = new Date(Date.now() - 3600000).toISOString().slice(0, 19).replace('T', ' '),
      to = new Date().toISOString().slice(0, 19).replace('T', ' '),
      action, service, ip, src_ip, dst_ip, device 
    } = req.query;
...
exports.getLiveEvents = async (req, res) => {
  try {
    const { last_id = 0, limit = 50 } = req.query;
    const eventsRes = await db.query('SELECT * FROM fw_events WHERE id>$1 ORDER BY id ASC LIMIT $2', [Number(last_id), Number(limit)]);
