...
exports.getSources = async (req, res) => {
  try {
    const sourcesRes = await db.query('SELECT * FROM fw_sources ORDER BY created DESC');
...
exports.addSource = async (req, res) => {
  try {
    ...
    const existingRes = await db.query('SELECT id FROM fw_sources WHERE log_path = $1', [log_path]);
...
exports.toggleSource = async (req, res) => {
  try {
    ...
    const currentRes = await db.query('SELECT * FROM fw_sources WHERE id = $1', [id]);
    ...
    await db.query('UPDATE fw_sources SET enabled = $1 WHERE id = $2', [newVal, id]);
...
exports.deleteSource = async (req, res) => {
  try {
    ...
    const infoRes = await db.query('DELETE FROM fw_sources WHERE id = $1', [id]);
