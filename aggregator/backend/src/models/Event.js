...
class Event {
  static async getAdAttacks(machine = '', limit = 3000, from, to) {
  ...
  static async getMaliciousEvents(machine = '', limit = 3000, from, to) {
    let sql = `SELECT e.id, e.machine, e.ts, e.tag, e.severity, e.category, e.message,
                      i.id as incident_id, i.assigned_to as incident_assigned_to, i.status as incident_status
      FROM events e
      LEFT JOIN incident_events ie ON ie.event_id = e.id
      LEFT JOIN incidents i ON i.id = ie.incident_id
      WHERE e.ts>=$1 AND e.ts<=$2 AND e.is_noise=0 AND e.category NOT IN ('DOMAIN','ADCS')
  ...
  static async getUsbEvents(machine = '', limit = 500, from, to) {
    let sql = `SELECT machine,ts,tag,severity,message FROM events
  ...
  static async getUserEvents(machine = '', limit = 500, from, to) {
    let sql = `SELECT machine,ts,tag,severity,message FROM events WHERE ts>=$1 AND ts<=$2 AND is_noise=0
