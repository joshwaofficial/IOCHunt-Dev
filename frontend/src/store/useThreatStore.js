...
export const useThreatStore = create((set, get) => ({
  ...
  connectSSE: (machine = '', range = 168, aggregator = '') => {
    ...
    sseSource.addEventListener('new_event', (event) => {
      try {
        ...
        if (data) {
          // 1. Check if the event matches the current machine and aggregator filter
          ...
          if (aggregator && aggregator !== 'All Aggregators' && data.aggregator_name !== aggregator) {
            return; // Skip this event, it's not from the filtered aggregator
          }
