...
export default function MfaSetup() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'Inter, sans-serif' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'Inter, sans-serif', padding: '20px' }}>
      <div style={{ background: 'var(--surface)', width: '100%', maxWidth: '420px', borderRadius: '16px', border: '1px solid var(--border)', padding: '40px', boxShadow: '0 24px 48px rgba(0,0,0,0.1)', position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <div style={{ fontSize: '24px', fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text)' }}>Enable MFA</div>
          <div style={{ fontSize: '12px', color: 'var(--muted)', fontFamily: 'JetBrains Mono, monospace', marginTop: '4px' }}>Scan with your Authenticator app</div>
        </div>
        ...
        {qrDataUrl && (
          <div style={{ background: '#fff', padding: '16px', borderRadius: '12px', margin: '24px auto', width: 'fit-content', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
            <img src={qrDataUrl} alt="QR Code" style={{ display: 'block', width: '200px', height: '200px' }} />
          </div>
        )}

        {secret && (
          <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', padding: '12px', borderRadius: '8px', textAlign: 'center', fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', letterSpacing: '2px', color: '#06b6d4', marginBottom: '24px' }}>
            {secret}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: 'var(--muted)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Verify Code</label>
