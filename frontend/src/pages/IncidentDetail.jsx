    ...
    <div style={{ width: '100%', paddingBottom: '40px', position: 'relative' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '30px', boxShadow: '0 8px 30px rgba(0,0,0,0.05)' }}>
        ...
        {incident.description && (
          <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px 20px', marginBottom: '24px', fontSize: '14px', color: 'var(--text)', lineHeight: 1.7, fontFamily: 'var(--sans)', whiteSpace: 'pre-wrap' }}>
            {incident.description}
          </div>
        )}

        {/* Key details row */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px', marginBottom: '24px', fontSize: '12px', fontFamily: 'var(--mono)' }}>
        ...
        {canTransition && INC_TRANSITIONS[incident.status?.toLowerCase()] && (
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '30px', padding: '16px 20px', background: 'var(--surface2)', border: '1px dashed var(--border)', borderRadius: '8px' }}>
            <span style={{ fontSize: '12px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', fontFamily: 'var(--mono)', fontWeight: 700, marginRight: '8px' }}>Change Status:</span>
            {INC_TRANSITIONS[incident.status?.toLowerCase()].map(ns => {
              ...
              return (
                <button 
                  ...
                  style={{ background: `${tm.col}1a`, border: `1px solid ${tm.col}44`, color: tm.col, padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 700, fontFamily: 'var(--mono)', transition: 'all 0.2s', textTransform: 'uppercase', letterSpacing: '0.5px' }}
        ...
        {closingTargetStatus && (
          <div style={{ 
            ...
            <div style={{ fontSize: '14px', fontWeight: 700, color: closingTargetStatus === 'resolved' ? '#22c55e' : 'var(--text)', marginBottom: '20px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
              <button onClick={() => setClosingTargetStatus(null)} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', padding: '8px 20px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 600, transition: 'all 0.2s' }}>Cancel</button>
              <button 
                ...
                style={{ background: closingTargetStatus === 'resolved' ? '#22c55e' : '#4a5578', border: 'none', color: '#fff', padding: '8px 24px', borderRadius: '6px', cursor: !resReason ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: 600, opacity: !resReason ? 0.5 : 1, transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
              >
                Confirm {closingTargetStatus}
              </button>
            </div>
        ...
        <div style={{ marginBottom: '30px' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: '12px', fontWeight: 700, color: 'var(--text)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span className="material-symbols-outlined" style={{ fontSize: '18px', color: 'var(--accent)' }}>link</span> Linked Events <span style={{ background: 'var(--surface2)', border: '1px solid var(--border)', padding: '2px 8px', borderRadius: '4px', fontSize: '10px' }}>{incident.events?.length || 0}</span>
          </div>
        ...
        <div style={{ marginBottom: '12px' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: '12px', fontWeight: 700, color: 'var(--text)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span className="material-symbols-outlined" style={{ fontSize: '18px', color: 'var(--accent)' }}>forum</span> Timeline & Notes <span style={{ background: 'var(--surface2)', border: '1px solid var(--border)', padding: '2px 8px', borderRadius: '4px', fontSize: '10px' }}>{incident.notes?.length || 0}</span>
          </div>
          
          {incident.notes?.length > 0 && (
            <div style={{ borderLeft: '2px solid var(--border)', marginLeft: '16px', paddingLeft: '28px' }}>
              {incident.notes.map(n => {
                const isSystem = n.note_type === 'system';
                return (
                  <div key={n.id} style={{ position: 'relative', marginBottom: '24px' }}>
                    <div style={{ position: 'absolute', left: '-35px', top: '6px', width: '12px', height: '12px', borderRadius: '50%', background: isSystem ? 'var(--surface2)' : 'var(--accent)', border: '2px solid var(--surface)', boxShadow: `0 0 0 2px ${isSystem ? 'var(--border)' : 'rgba(37,99,235,0.3)'}` }}></div>
                    ...
                    <div style={{ background: isSystem ? 'var(--surface2)' : 'rgba(37,99,235,0.05)', border: `1px solid ${isSystem ? 'var(--border)' : 'rgba(37,99,235,0.2)'}`, padding: '16px 20px', borderRadius: '0 8px 8px 8px', fontSize: '13px', color: 'var(--text)', lineHeight: 1.6, fontFamily: 'var(--sans)', fontStyle: isSystem ? 'italic' : 'normal', whiteSpace: 'pre-wrap' }}>
                      {n.body}
                    </div>
                  </div>
                );
          ...
          <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px dashed var(--border)' }}>
            <textarea 
              ...
              style={{ width: '100%', background: 'transparent', border: '2px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--sans)', fontSize: '14px', padding: '14px 18px', borderRadius: '8px', resize: 'vertical', lineHeight: 1.6, marginBottom: '16px', outline: 'none', transition: 'border-color 0.2s' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button 
                ...
                style={{ background: 'var(--accent)', color: '#fff', border: '1px solid var(--accent)', padding: '10px 24px', borderRadius: '6px', fontSize: '13px', fontWeight: 700, fontFamily: 'var(--mono)', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '1px', boxShadow: '0 4px 12px rgba(37,99,235,0.2)', transition: 'background 0.2s', opacity: (addNoteMutation.isPending || !newNote.trim()) ? 0.5 : 1 }}
