...
export default function AdAttacks() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const params = { machine, page, limit: perPage, search: searchTerm, severity: severityFilter, aggregator: branchFilter || aggregator || undefined, actor: actorFilter, attackType: typeFilter, process: typeFilter, action: typeFilter, sort: sortFilter, protocol: protocolFilter, tactic: tacticFilter };
        if (range === 'custom') {
        ...
        const res = await axios.get('/api/events/ad-attacks', { params });
    ...
    fetchData();
  }, [range, machine, page, perPage, searchTerm, severityFilter, customFrom, customTo, actorFilter, typeFilter, sortFilter, protocolFilter, tacticFilter, branchFilter, aggregator]);

  // Frontend filtering applied to the current page's data
  const filteredData = data;

  const total = serverStats.total || filteredData.length;
  const crit = serverStats.critical || 0;
  const high = serverStats.high || 0;
  
  // These rely on the loaded data
  const uniqueTypes = new Set(filteredData.map(a => a.attack_type)).size;
  const uniqueMachines = new Set(filteredData.map(a => a.target_machine || a.machine)).size;

  const totalPages = Math.max(1, Math.ceil(total / perPage));
  const currentPage = Math.min(page, totalPages);
  
  // Data is already paginated by the backend, so we just use the filtered results directly
  ...
  return (
    <div style={{ width: '100%', paddingBottom: '40px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '26px', fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text)', margin: 0 }}>AD Attack Intelligence</h2>
          <p style={{ fontSize: '11px', color: 'var(--muted)', margin: '6px 0 0', fontFamily: 'var(--mono)' }}>Detect and analyze Active Directory threats, lateral movement, and privilege escalation.</p>
        </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '14px 20px', display: 'flex', flexWrap: 'nowrap', alignItems: 'center', gap: '12px', marginBottom: '20px', justifyContent: 'space-between', overflowX: 'auto' }}>
        ...
        <select 
          ...
          style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '6px', padding: '6px 10px', fontSize: '11px', color: 'var(--text)', outline: 'none', cursor: 'pointer', fontFamily: 'var(--sans)' }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        ...
        <PremiumCard value={uniqueTypes} label="Attack Types" color="#eab308" icon="key" subtitle="Unique vectors" />
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="material-symbols-outlined" style={{ fontSize: '18px', color: 'var(--muted)' }}>shield_lock</span>
          <h2 style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--text)', fontFamily: 'var(--mono)', margin: 0 }}>Active AD Threats</h2>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--muted)' }}>No AD attacks in this window</div>
        ) : (
          <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--mono)' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(90deg, rgba(37,99,235,0.06) 0%, rgba(37,99,235,0) 100%)' }}>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'left' }}>Type</th>
                ...
                <tbody>
                  {paginatedData.map((a, i) => {
                    ...
                    const typCol = adCol(a.attack_type);
                    ...
                    return (
                      <tr key={i} className="hover-row" onClick={() => setSelectedEvent(a)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                        ...
                        <td style={{ padding: '14px 16px', fontSize: '12px', fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--sans)' }}>
                          ...
                          {esc(a.attack_type || a.tag || 'Unknown')}
                        </td>
                        <td style={{ padding: '14px 16px' }}>
                        ...
                        <td style={{ padding: '14px 16px', fontSize: '11px', color: 'var(--muted)' }}>{esc(a.protocol || '')}</td>
            ...
            <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
              <span style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                Showing page {currentPage} of {total} entries
              </span>
