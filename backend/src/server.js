...
// Intercept GET /api/aggregators on branch nodes to return empty array instead of 403 (prevents console spam)
app.get('/api/aggregators', (req, res, next) => {
  if (!appMode.isCentralServer()) return res.json({ data: [] });
  next();
});
