...
const generateReport = async (req, res) => {
  try {
    ...
    const criticalEvents = (await db.query(`SELECT machine, ts, tag, category, severity, message FROM events ${evWhere.replace('is_noise=false', "is_noise=false AND severity IN ('critical','high')")} ORDER BY ts DESC LIMIT 300`, evParams)).rows;
